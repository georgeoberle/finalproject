...
year <- ggplot(socdata, aes(x=Year.Elected, fill= Organization ))
year + geom_histogram(position = "dodge", binwidth = 1) +
ggtitle("Annual Elected Members of APS and AAAS") + ylab("Total Members Elected") + xlab("Year Elected")+facet_wrap("Year.Elected")
...
#How do I label the total number?
...
a <- ggplot(dataaps, aes(x=Year.Elected, fill=Residency))
a + geom_bar(position = "dodge", binwidth = 1)+ xlab("Year Elected")+ggtitle ("Annual Elected Members APS by Residency")+facet_wrap("Year.Elected")
...
#State of socities before 1800 filter by Date.Elected
socdata %>%
  filter(Year.Elected < 1800) %>%
  graph.data.frame(directed = FALSE) %>%
  plot(vertex.label = NA)


#Then run functions for bipartite
...
z <- graph.data.frame(test)
adj <- get.adjacency(z, sparse = FALSE)
adj
adj_simple <- adj[rownames(adj) %in% test$people, colnames(adj) %in% test$orgs]
adj_simple
as.data.frame(adj_simple)

#My Application of Example
newsocdata <-graph.data.frame(socdata)
apsaaas<- get.adjacency(newsocdata, sparse= FALSE)
apsaaas <- apsaaas[rownames(apsaaas) %in% socdata$Name, colnames(apsaaas) %in% socdata$Organization]

# Query membership for a particular person
apsaaas[rownames(apsaaas) == "John Adams", ]

#Bipartite map of APS and AAAS need to update this.
V(data_g)
bipartite.mapping(data_g)
V(data_g)$type <- bipartite.mapping(data_g)$type
is.bipartite(data_g)
...
schol_soc_geocoded
write.csv(schol_soc_geocoded, file = "shol-soc-geocoded.csv")
...
early_colleges_geocoded <- read.csv("early_colleges_geocoded.csv")
l <- ggplot(early_colleges_geocoded, aes(x=state, fill=type))
l + geom_histogram (position = "dodge", binwidth = 1)+ xlab("State Founded")+ggtitle ("Early American Colleges by Sponsorship")
